...
const macroChecks = (facts, th) => {
    ...
    out.push(check({
        ...
        hint: 'macroDataService.updateCurrencies — cadeia Yahoo → Coinbase (só BTC) → PTAX/BCB (só dólar, e só a fixação do dia) → Coinbase taxas (as duas moedas, variação derivada por nós), a cada 15 min. Câmbio parado congela a conversão de toda posição dolarizada, não só a barra de indicadores.',
    }));
