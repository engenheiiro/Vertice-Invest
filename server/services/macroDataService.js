...
export const macroDataService = {
    ...
    async updateTreasuryRates() {
        ...
        if (uniqueBonds.length >= 5) {
            logger.info("🧹 [Tesouro] Limpando base antiga de títulos...");
            await TreasuryBond.deleteMany({});
            
            logger.info(`💾 [Tesouro] Persistindo ${uniqueBonds.length} títulos atualizados...`);
    ...
    async performMacroSync() {
        ...
        const globalIndices = await externalMarketService.getGlobalIndices(); // Busca IBOV e SPX
        ...
        // PERSISTÊNCIA DE MOEDAS E ÍNDICES PARA CACHE
        if (currencies) {
        ...
        logger.info(`✅ [MACRO] Sync Finalizado.`);
        return config;
    }
};
