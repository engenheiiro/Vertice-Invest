...
if (!EMAIL) { console.error('Uso: node auditWallet.js email'); process.exit(1); }
...
const run = async () => {
  ...
  console.log(`criado: ${d(user.createdAt)}`);
  ...
  // ---- TRANSAÇÕES ----
  const txs = await Tx.find({ user: user._id }).sort({ date: 1, createdAt: 1 }).lean();
  ...
  // ---- SNAPSHOTS ----
  const snaps = await Snap.find({ user: user._id }).sort({ date: 1 }).lean();
