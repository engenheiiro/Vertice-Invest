...
export const AllocationChart = () => {
    ...
    // 1. Calcular Valores Atuais por Categoria
    ...
    // 2. Separar Reserva do Total de Investimentos para cálculos de %
    ...
    // 3. Gerar Dados para o Gráfico (Excluindo CASH)
    let activeDataForChart: Partial<Record<AssetType, number>> = {};
    
    if (viewMode === 'CURRENT') {
        ORDERED_TYPES.filter(t => t !== 'CASH').forEach(type => {
    ...
    return (
        <div className="bg-[#080C14] border border-slate-800 rounded-2xl p-6 h-[380px] flex flex-col relative">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex gap-1 bg-slate-900 p-1 rounded-lg border border-slate-800">
                        <button 
                            ...
                            className={`text-[10px] font-bold px-3 py-1 rounded transition-colors ${viewMode === 'CURRENT' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                        >
                            Atual
                        </button>
                        <button 
                            onClick={() => setViewMode('IDEAL')}
                            className={`text-[10px] font-bold px-3 py-1 rounded transition-colors ${viewMode === 'IDEAL' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                        >
                            Ideal
                        </button>
                    </div>
                    ...
                    <button 
                        ...
                        className="p-1.5 text-slate-500 hover:text-white transition-colors rounded-lg hover:bg-slate-800 border border-transparent hover:border-slate-700"
                        title="Configurar Carteira Ideal"
                    >
                        <Settings size={14} />
                    </button>
                </div>
            ...
            {/* Conteúdo: Layout Flex Row (Lado a Lado) */}
            <div className="flex items-center gap-4 h-full">
                ...
                {/* Lista / Legenda Lateral com Scroll se necessário */}
                <div className="flex-1 h-full overflow-y-auto custom-scrollbar pr-2 flex flex-col justify-center">
                    {ORDERED_TYPES.map((type) => {
                        ...
                        if (isCash) {
                            const currentR = currentValues['CASH'];
                        ...
                        if (!isCash && (currentValues[type] <= 0 && viewMode === 'CURRENT')) return null;
                        ...
                        return (
                            <div key={type} className="flex justify-between items-center text-xs py-1.5 border-b border-slate-800/30 last:border-0">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <span className={`font-medium truncate ${isCash ? 'text-slate-200' : 'text-slate-400'}`}>
                                        {LABELS[type]}
                                    </span>
                                </div>
            ...
            {isEditing && (
                <div className="absolute inset-0 bg-[#080C14] z-20 flex flex-col p-6 animate-fade-in rounded-2xl">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-5 custom-scrollbar pr-1">
                        ...
                        <div className="bg-slate-900/30 p-3 rounded-xl border border-slate-800">
                            <h5 className="text-xs font-bold text-slate-400 uppercase mb-2 flex items-center gap-1">
                                <DollarSign size={10} /> Reserva (Valor Fixo)
                            </h5>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-800 flex justify-between items-center">
                        ...
                        <Button className="w-auto py-1.5 px-4 text-xs" onClick={saveTargets}>
