                ...
                <div className="grid md:grid-cols-3 gap-6 lg:gap-8 items-start">
                    ...
                    <PricingCard 
                        ...
                        features={[
                            ...
                            { text: "Sinais IA (Delay 7 dias)", available: true },
                            { text: "Cursos Nível Iniciante", available: true },
                            { text: "Aporte Inteligente", available: false },
                    ...
                    <div className="relative z-10">
                        ...
                        <PricingCard 
                            ...
                            features={[
                                ...
                                { text: "Aporte Inteligente (Ilimitado)", available: true, highlight: "Novo" },
                                ...
                                { text: "Rebalanceamento Automático", available: false },
                            ]}
                            isPopular
                    ...
                    <PricingCard 
                        ...
                        features={[
                            ...
                            { text: "Rebalanceamento IA Automático", available: true, highlight: "Exclusivo" },
    ...
    <div className={`bg-[#080C14] border ${borderColor} rounded-2xl p-8 relative overflow-hidden flex flex-col h-full transition-all duration-300 ${isPopular ? 'shadow-2xl shadow-blue-900/10 ring-1 ring-blue-500/30 bg-[#0B101A]' : hoverColor}`}>
        ...
        <div className="mb-6 relative z-10">
            <div className="flex items-center justify-between mb-4">
                ...
                {isPopular && <span className="text-[10px] font-bold uppercase tracking-wider text-blue-400 bg-blue-900/20 px-2.5 py-1 rounded border border-blue-900/30">Recomendado</span>}
            </div>
            ...
            {features.map((feature: any, idx: number) => (
                <div key={idx} className={`flex items-start gap-3 text-sm leading-relaxed ${feature.available ? 'text-slate-300' : 'text-slate-600'}`}>
                    ...
                    <span className={`flex-1 ${!feature.available && 'line-through decoration-slate-700'}`}>
                        {feature.text}
                        {feature.highlight && <span className="ml-2 text-[10px] font-bold text-emerald-500 uppercase inline-block border border-emerald-500/20 bg-emerald-500/5 px-1.5 rounded">{feature.highlight}</span>}
                    </span>
                </div>
            ))}
        </div>
        <div className="relative z-10 mt-auto">
            {current ? (
                <div className="w-full py-4 rounded-xl bg-slate-800/50 border border-slate-700 text-slate-400 text-sm font-bold text-center cursor-default flex items-center justify-center gap-2"><Check size={16} /> Seu Plano Atual</div>
            ) : (
                <Button variant={buttonVariant} className="w-full text-xs uppercase tracking-wide py-4" onClick={() => onSelect(id)} status={isLoading ? 'loading' : 'idle'}>
