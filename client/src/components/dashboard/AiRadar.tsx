...
import { Radar, Zap, Lock, TrendingUp, AlertTriangle, Shield, Activity, Crown, Search, Info, History, Filter, Sparkles, BookOpen } from 'lucide-react';
...
export const AiRadar: React.FC<AiRadarProps> = ({ signals, isLoading = false }) => {
    ...
    const filteredSignals = useMemo(() => {
        if (!hasAccess) return signals.slice(0, 3); // Sem acesso vê apenas 3 (dummy/antigos)
        
        return signals.filter(s => {
            if (filter === 'ALL') return true;
            return s.assetType === filter;
        });
    ...
    // Handler de Navegação Direta
    const handleSignalClick = (ticker: string) => {
        if (!hasAccess) return;
        // Navega para Research passando o ticker no state para auto-abertura
        navigate('/research', { state: { openTicker: ticker } });
    };

    const handleHistoryClick = () => {
        navigate('/radar'); // Nova página de histórico
    };
    ...
    return (
        <div className="bg-[#080C14] border border-slate-800 rounded-2xl flex flex-col h-[450px] relative group">
            ...
            <div className="p-4 border-b border-slate-800 bg-[#0B101A] flex items-center justify-between rounded-t-2xl">
                <div className="flex items-center gap-2">
                    ...
                    <div className="group/tooltip relative flex items-center z-50">
                        ...
                        <div className="absolute left-0 top-6 w-56 p-3 bg-[#0F1729] border border-slate-700 rounded-xl shadow-2xl z-50 opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none text-left">
                            <p className="text-[10px] text-slate-300 leading-relaxed">
                                O Radar Alpha usa matemática (RSI, Graham, Volumetria) para encontrar anomalias.
                                <br/><span className="text-emerald-400">Deep Value:</span> Preço abaixo do justo.
                                <br/><span className="text-blue-400">RSI Oversold:</span> Desconto técnico.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {/* Lista com Scroll e Rounded Bottom */}
            <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar bg-gradient-to-b from-[#080C14] to-[#05070a] relative rounded-b-2xl">
                            ...
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex flex-col gap-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {/* BADGE DE FONTE (ALGO vs RESEARCH) */}
                                        {isAlgo ? (
                ...
                {hasAccess && isEmpty && (
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-50 py-10">
                        ...
                        <p className="text-sm font-bold text-slate-500">Nenhuma anomalia detectada.</p>
