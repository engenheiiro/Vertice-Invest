    ...
    <div className="min-h-screen bg-slate-50/50">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex items-center bg-slate-100 rounded-full px-3 py-1.5 border border-slate-200 focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-400 transition-all">
                ...
                <input type="text" placeholder="Buscar ativo..." className="bg-transparent border-none outline-none text-xs ml-2 w-24 text-slate-600 placeholder:text-slate-400" />
            </div>
            ...
            <button 
              ...
              className="text-sm font-semibold text-slate-600 hover:text-red-600 flex items-center gap-2 transition-colors px-3 py-2 hover:bg-red-50 rounded-lg"
            >
              <LogOut size={16} />
              Sair
            </button>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Painel de IA Research</h1>
                <p className="text-slate-500 mt-2">Insights diários gerados por modelos de <span className="text-blue-600 font-semibold">Deep Learning</span>.</p>
            </div>
            ...
            <div className="col-span-1 md:col-span-4 space-y-8">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    ...
                    <div className="space-y-4">
                        <StockRow ticker="NVDA" name="NVIDIA Corp" change="+2.4%" price="845.20" positive />
                        <div className="h-px bg-slate-100"></div>
                        <StockRow ticker="TSLA" name="Tesla Inc" change="-1.2%" price="172.50" positive={false} />
                        <div className="h-px bg-slate-100"></div>
                        <StockRow ticker="AMD" name="Advanced Micro" change="+0.8%" price="164.10" positive />
                    </div>
                </div>
            </div>
        </div>
...
const NavItem = ({ icon, label, active = false, disabled = false }: any) => (
  <button 
    disabled={disabled}
    className={`
...
const Card = ({ title, value, sub, icon }: any) => (
...
const StockRow = ({ ticker, name, change, positive, price }: any) => (
